...
public abstract class AbstractProtoSerializer implements Serializer,
		Deserializer {
	...
	public <T> byte[] serialize(final T source) {
		...
		final LinkedBuffer buffer = LinkedBuffer
	...
	public <T> T deserialize(final byte[] bytes, final Class<T> clazz) {
		try {
			...
			return deserializeInternal(bytes, result,
					SchemaUtils.getSchema(clazz));
		} catch (final Exception e) {
			throw new IllegalStateException(e);
		}
	}

	public abstract <T> byte[] serializeInternal(final T source,
			final Schema<T> schema, final LinkedBuffer buffer);

	public abstract <T> T deserializeInternal(final byte[] bytes,
			final T result, final Schema<T> schema);
}
